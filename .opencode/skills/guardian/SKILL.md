...
- **every commit** — always ask the user to confirm before running `git commit`, `git push`, or any commit-creating command; never auto-commit
...
- **all commits**: always ask the user to confirm before executing any `git commit`, `git commit --amend`, `git squash`, or other commit-creating command — never auto-commit without explicit user consent
