...
## Mandatory Commit Verification Gate (applies in EVERY mode)

**Never commit, push, or open a PR without explicit user approval — even in `AUTORUN` / `AUTORUN_FULL`.**

Agent autonomy governs task execution; it does **NOT** extend to committing to the user's repository without their consent.

Required workflow:
1. **STOP** — do not run `git commit`, `git push`, or `gh pr create` yet.
2. Show the user `git status` and the exact list of files you intend to stage.
3. State the proposed commit message (and any PR title/body).
4. **Wait for an explicit yes/no** from the user.
5. Only after approval: stage, commit, push, and open the PR.

Rules:
- If the project's `AGENTS.md` says "always ask the user before committing", treat it as non-negotiable and confirm **every single time** (do not commit once and assume it covers later commits).
- Stage only intended files; never include unrelated files (e.g. `backend/logs/*.log`) without flagging them.
- Only skip the gate when the user has **explicitly requested** the commit/PR in the current task.
