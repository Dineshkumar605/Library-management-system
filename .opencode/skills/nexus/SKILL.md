...
1. Never read the whole repository unless absolutely necessary.
...
3. Identify the type of request:
   ...
   - test
   - review
4. Search for relevant classes, methods, APIs, errors, and filenames.
5. Use Lens for code understanding.
6. Use Scout for bug investigation.
7. Pass only relevant findings to the next agent.
8. Do not pass full previous conversations between agents.
9. Skip agents that are not required.
10. Expand to additional files only when the current files are insufficient.